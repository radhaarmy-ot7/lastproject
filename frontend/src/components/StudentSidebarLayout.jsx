...
const StudentSidebarLayout = ({ children }) => {
  ...
  const navItems = [
    ...
    { path: '/student/search', icon: FiSearch, label: 'Search Student' },
    { path: '/student/attendance', icon: FiCalendar, label: 'My Attendance' },
    { path: '/student/results', icon: FiBarChart2, label: 'My Results' },
    { path: '/student/notices', icon: FiBookOpen, label: 'Notices' },
  ];
          ...
          <div className="flex items-center gap-2">
            ...
            {isSidebarOpen && (
              <div className="transition-opacity duration-300">
                <h1 className="font-bold text-green-700 text-sm">KV School</h1>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`sidebar-link ${isActive ? 'active border-green-600' : ''} ${
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            {isSidebarOpen && (
              <div className="flex-1 min-w-0 transition-opacity duration-300">
                ...
                <p className="text-xs text-secondary-500 capitalize">{user?.role || 'Student'}</p>
                <p className="text-xs text-secondary-400">Admission: {user?.admission_number || 'N/A'}</p>
              </div>
            )}
          </div>
      ...
      <div className={`flex-1 ${isSidebarOpen ? 'md:ml-64 ml-0' : 'md:ml-20 ml-0'} transition-all duration-300`}>
        ...
        <header className="bg-white border-b border-secondary-200 px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center text-green-700 font-semibold sm:hidden">
              {user?.full_name?.charAt(0) || 'S'}
            </div>
          </div>
        </header>
